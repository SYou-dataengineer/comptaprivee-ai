"""Formulaire 3A réel, invalidation et rechargement."""
...
def ouvrir(app,confirme=True):
    tax_case_storage.sauvegarder_dossier_fiscal(dossier_interets(),profil_interets=replace(profil_interets(),confirme=confirme))
    ...
    bouton(fiscal,'Intérêts canadiens 2025').invoke()
...
def test_confirmation_export_stockage_invalidation(application,monkeypatch,tmp_path):
    ...
    champ(dialogue,'confirmation_interets').invoke();bouton(dialogue,'Valider et appliquer').invoke()
    ...
    assert 'INTÉRÊTS CANADIENS' in texte and '12100' in texte and '446' in texte
    ...
    bouton(fiscal,'Intérêts canadiens 2025').invoke();dialogue=derniere_fenetre(fiscal)
    champ(dialogue,'source_interets').insert(0,'Revu : ')
    check=champ(dialogue,'confirmation_interets');assert dialogue.getvar(check.cget('variable'))==0
    ...
    bouton(fiscal,'Dossiers enregistrés').invoke();bouton(derniere_fenetre(fiscal),'Ouvrir le dossier').invoke()
    bouton(fiscal,'Intérêts canadiens 2025').invoke();dialogue=derniere_fenetre(fiscal)
    assert champ(dialogue,'source_interets').get().startswith('Revu : ')

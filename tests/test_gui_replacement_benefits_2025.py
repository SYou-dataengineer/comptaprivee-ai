"""Formulaire 2H réel, invalidation et rechargement."""
...
from tests.test_tax_replacement_benefits_2025 import dossier_remplacement, profil_remplacement, NATURES_REMPLACEMENT


def ouvrir(app,confirme=True,nature='CNESST'):
    tax_case_storage.sauvegarder_dossier_fiscal(dossier_remplacement(nature),profil_remplacement=replace(profil_remplacement(nature),confirme=confirme))
    ...
    bouton(fiscal,'Autres prestations 2025').invoke()
...
def test_confirmation_export_stockage_invalidation(application,monkeypatch,tmp_path,nature):
    ...
    champ(dialogue,'confirmation_remplacement').invoke();bouton(dialogue,'Valider et appliquer').invoke()
    ...
    assert 'PRESTATIONS DE REMPLACEMENT' in texte and '14400' in texte and '358' in texte
    ...
    assert tax_case_storage.charger_dossier_fiscal(chemin).profil_remplacement.confirme
    bouton(fiscal,'Autres prestations 2025').invoke();dialogue=derniere_fenetre(fiscal)
    champ(dialogue,'source_remplacement').insert(0,'Revu : ')
    check=champ(dialogue,'confirmation_remplacement');assert dialogue.getvar(check.cget('variable'))==0
    ...
    bouton(fiscal,'Dossiers enregistrés').invoke();bouton(derniere_fenetre(fiscal),'Ouvrir le dossier').invoke()
    bouton(fiscal,'Autres prestations 2025').invoke();dialogue=derniere_fenetre(fiscal)
    assert champ(dialogue,'source_remplacement').get().startswith('Revu : ')
...
def test_import_preparation(application,monkeypatch,vide):
    ...
    assert 'T5007 : 1' in app.messages_test[-1][1]

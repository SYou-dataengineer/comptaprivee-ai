"""Parcours PSV et suppléments avec feuillets PSV et suppléments fictifs et stockage temporaire."""
...
from tests.test_tax_old_age_security_2025 import dossier_psv


def ouvrir(app, confirme=False):
    tax_case_storage.sauvegarder_dossier_fiscal(dossier_psv(), psv_confirme=confirme)
    ...
    bouton(fiscal, "PSV et suppléments 2025").invoke()
    return fiscal, derniere_fenetre(fiscal)


@pytest.mark.parametrize("echelle", [96/72, 144/72, 192/72])
def test_psv_actions_visibles(application, echelle):
    app = application
...
def test_psv_confirmation_calcul_export_et_rechargement(application, monkeypatch, tmp_path):
    app = application
    fiscal, dialogue = ouvrir(app)
    bouton(dialogue, "Valider et appliquer").invoke()
    assert dialogue.winfo_exists() and app.messages_test[-1][0] == "PSV et suppléments invalide"
    ...
    assert "11300" in texte and "63\u00a0515,00" in texte
    pdf = tmp_path / "psv.pdf"
    ...
    assert charge.psv_confirme and not charge.rqap_confirme
...
@pytest.mark.parametrize("vide", [None, "T4A(OAS)"])
def test_psv_import_validation_et_preparation(application, monkeypatch, vide):
    app = application
    fiscal, dialogue = ouvrir(app)
    bouton(dialogue, "Fermer").invoke()
    dossier = dossier_psv()
    ...
    assert "T4A(OAS) : 1" in app.messages_test[-1][1]
...
def test_psv_sans_dossier(application):
...
@pytest.mark.parametrize('pension,supplements', [('10000','0'),('10000','12000'),('0','12000')])
def test_psv_gui_sans_emploi(application, monkeypatch, tmp_path, pension, supplements):
    app=application
    tax_case_storage.sauvegarder_dossier_fiscal(dossier_psv(emploi=False,pension=pension,supplements=supplements),psv_confirme=True)
    app.ouvrir_agent_fiscal()
    fiscal=derniere_fenetre(app)
    bouton(fiscal,'Dossiers enregistrés').invoke()
    bouton(derniere_fenetre(fiscal),'Ouvrir le dossier').invoke()
    ...
    resultat=derniere_fenetre(fiscal)
    texte=next(w for w in descendants(resultat) if isinstance(w,tk.Text)).get('1.0','end')
    assert '11300' in texte and '25000' in texte

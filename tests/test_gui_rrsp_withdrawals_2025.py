"""Formulaire 2F réel, invalidation et rechargement."""
...
from tests.test_tax_rrsp_withdrawals_2025 import dossier_retraits, profil_retraits, NATURES_RETRAITS


def ouvrir(app,confirme=True,nature='REER_ORDINAIRE'):
    tax_case_storage.sauvegarder_dossier_fiscal(dossier_retraits(nature),profil_retraits=replace(profil_retraits(nature),confirme=confirme))
    ...
    bouton(fiscal,'Retraits REER et forfaits 2025').invoke()
...
def test_confirmation_export_stockage_invalidation(application,monkeypatch,tmp_path,nature):
    ...
    champ(dialogue,'confirmation_retraits').invoke();bouton(dialogue,'Valider et appliquer').invoke()
    ...
    assert 'RETRAITS REER' in texte and '12900' in texte and '250 point 6' in texte
    ...
    assert tax_case_storage.charger_dossier_fiscal(chemin).profil_retraits.confirme
    bouton(fiscal,'Retraits REER et forfaits 2025').invoke();dialogue=derniere_fenetre(fiscal)
    champ(dialogue,'source_retraits').insert(0,'Revu : ')
    check=champ(dialogue,'confirmation_retraits');assert dialogue.getvar(check.cget('variable'))==0
    ...
    bouton(fiscal,'Dossiers enregistrés').invoke();bouton(derniere_fenetre(fiscal),'Ouvrir le dossier').invoke()
    bouton(fiscal,'Retraits REER et forfaits 2025').invoke();dialogue=derniere_fenetre(fiscal)
    assert champ(dialogue,'source_retraits').get().startswith('Revu : ')
...
def test_import_preparation(application,monkeypatch,vide):
    ...
    assert 'T4RSP : 1' in app.messages_test[-1][1]

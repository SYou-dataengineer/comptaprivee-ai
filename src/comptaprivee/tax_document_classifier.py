...
def classifier_document_fiscal(
    ...
    nouveaux = []
    for type_doc, motif in (("T5008", r"(?<![a-z0-9])t5008(?![a-z0-9])"), ("RL-18", r"(?<![a-z0-9])(?:rl[ _-]?18|releve[ _-]*18)(?![a-z0-9])"), ("INTERETS", r"\b(?:etat|releve|avis)[ _-]+interets[ _-]+2025\b"), ("RL-3", r"(?<![a-z0-9])(?:rl[ _-]?3|releve[ _-]*3)(?![a-z0-9])"), ("T5007", r"(?<![a-z0-9])t5007(?![a-z0-9])"), ("RL-5", r"(?<![a-z0-9])(?:rl[ _-]?5|releve[ _-]*5)(?![a-z0-9])"), ("T4RSP", r"(?<![a-z0-9])t4rsp(?![a-z0-9])"), ("T4A", r"(?<![a-z0-9])t4a(?![a-z0-9]|[ _-]*\(|[ _-]*(?:oas|p|rca)\b)"), ("T4RIF", r"(?<![a-z0-9])t4rif(?![a-z0-9])"), ("T3", r"(?<![a-z0-9])t3(?![a-z0-9])"), ("T5", r"(?<![a-z0-9])t5(?![a-z0-9])"), ("RL-16", r"(?<![a-z0-9])(?:rl[ _-]?16|releve[ _-]*16)(?![a-z0-9])"), ("T4A(OAS)", r"(?<![a-z0-9])t4a[ _-]*\(?oas\)?(?![a-z0-9])"), ("T4A(P)", r"(?<![a-z0-9])t4a[ _-]*\(?p\)?(?![a-z0-9])"), ("RL-2", r"(?<![a-z0-9])(?:rl[ _-]?2|releve[ _-]*2)(?![a-z0-9])"), ("T4E", r"(?<![a-z0-9])t4e(?![a-z0-9])"), ("RL-6", r"(?<![a-z0-9])(?:rl[ _-]?6|releve[ _-]*6)(?![a-z0-9])")):

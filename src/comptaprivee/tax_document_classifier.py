...
def classifier_document_fiscal(
    ...
    nouveaux = []
    for type_doc, motif in (("T4A(OAS)", r"(?<![a-z0-9])t4a[ _-]*\(?oas\)?(?![a-z0-9])"), ("T4A(P)", r"(?<![a-z0-9])t4a[ _-]*\(?p\)?(?![a-z0-9])"), ("RL-2", r"(?<![a-z0-9])(?:rl[ _-]?2|releve[ _-]*2)(?![a-z0-9])"), ("T4E", r"(?<![a-z0-9])t4e(?![a-z0-9])"), ("RL-6", r"(?<![a-z0-9])(?:rl[ _-]?6|releve[ _-]*6)(?![a-z0-9])")):
